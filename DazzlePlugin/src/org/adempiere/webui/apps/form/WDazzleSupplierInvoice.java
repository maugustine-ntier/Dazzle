 ...
 */
public class WDazzleSupplierInvoice extends WDazzleBase
implements IFormController, EventListener<Event>, WTableModelListener, ValueChangeListener,TableValueChangeListener
{
	...
	private static CLogger log = CLogger.getCLogger(WDazzleSupplierInvoice.class);
	...
	private Label lblPONo = new Label("Purchase Order No");
	private ComboboxNew cmbPONo = new ComboboxNew();
	...
	private void zkInit () {
				...
				public void onEvent(org.zkoss.zk.ui.event.Event event)
					...
					if (cmbCustomer.getValue() != null && cmbCustomer.getSelectedItem() != null) {
						...
						if (selIndex > -1) {
							...
							cmbPONo.setReadonly(false);
							cmbPONo.setDisabled(false);
							setComboPONo();
						}

						cmbPONo.setFocus(true);
					} else {
						...
						cmbPONo.setReadonly(true);
						cmbPONo.setDisabled(true);
		...
		if (!cmbPONo.isListenerAvailable(Events.ON_CHANGE, false)) {
			cmbPONo.addEventListener(Events.ON_CHANGE,new org.zkoss.zk.ui.event.EventListener() {
				public void onEvent(org.zkoss.zk.ui.event.Event event)
						throws Exception {
					//NCG 2014/09/08 Resolve problems with F6
					clearBottomScreen();
					//clearBottomScreen();
					//clearScreen();
					if (cmbPONo.getValue() != null && cmbPONo.getSelectedItem() != null) {
						int orderID = ((WObjRet)cmbPONo.getSelectedItem().getValue()).getIDOfRecord();
						populateScreen(orderID);
						cmbPONo.setFocus(true);
					} else {
...
//		if (!txtCustomerRefNo.isListenerAvailable(Events.ON_CHANGE, false)) {
//			txtCustomerRefNo.addEventListener(Events.ON_CHANGE,new org.zkoss.zk.ui.event.EventListener() {
//				public void onEvent(org.zkoss.zk.ui.event.Event event)
//						throws Exception {
//					throw new WrongValueException(txtCustomerRefNo, "The document number already exists in the invoice table.");
//				}
//			});
//		}
		...
		row = rows.newRow();
		row.appendChild(lblPONo);
		cmbPONo.setReadonly(true);
		cmbPONo.setDisabled(true);
	    row.appendChild(cmbPONo);
...
//						 //return;
//					 }
					 charge_ID_sel = getSelectedChargeID();
...
//							}
	...
	private void EnterPressedOnAmount() {
		...
		//NCG 2014/09/30 #1000356: Allow only a charge to be entered on a Supplier Invoice (pl->mCharge)
		BigDecimal tax = mTax.calculateTax(unitPrice,mCharge.isTaxIncluded(), 2);
	...
	private void setComboPONo() {
		//SimpleConstraint constr2 = new SimpleConstraint("");
		//cmbPONo.setConstraint(constr2);
		cmbPONo.setValue(null);
		//constr2 = new SimpleConstraint("no empty");
		//cmbPONo.setConstraint(constr2);
//		StringBuffer sql = new StringBuffer("SELECT o.Documentno || '_ R' || o.grandTotal,o.C_Order_ID " )
		StringBuffer sql = new StringBuffer("SELECT  o.Documentno || '_ ' || to_char(o.dateordered, 'YYYY-MM-DD') || '_ R' || o.grandTotal,o.C_Order_ID " )
				...
				+ "WHERE o.C_BPartner_ID= " + currentBPartner.getC_BPartner_ID()
				+ " and o.documentno like ?"
				+ " AND o.IsSOTrx='N' AND o.DocStatus IN ('CL','CO')"
				+ " and o.ad_client_Id = "	+ Env.getAD_Client_ID(m_ctx)
				+ " and o.isactive='Y' "
				+ " AND o.C_Order_ID IN "
		...
		dictModel2.setCbox(cmbPONo);
		cmbPONo.setModel(dictModel2);
		cmbPONo.setQueryName(sql2);
		cmbPONo.setAutodrop(true);
		cmbPONo.setButtonVisible(true);
		cmbPONo.setWidth("400px");
	}

	private void populateScreen(int c_Order_ID) {

		mOrder = new MOrder(m_ctx, c_Order_ID, null);

		if (mOrder.getPOReference() != null && !mOrder.getPOReference().equals(null)) {
			txtCustomerRefNo.setValue(mOrder.getPOReference());
		}
		if (mOrder.getDescription() != null && !mOrder.getDescription().equals(null)) {
			String desc = mOrder.getDescription();
			txtComment.setValue(desc);
		}


        headerload();
		int ids [] = PO.getAllIDs("C_OrderLine", "C_Order_ID = " + mOrder.getC_Order_ID() + " order by line" , null);
		for (int i = 0; i < ids.length; i++) {
			MOrderLine mOrderLine = new MOrderLine(m_ctx,ids[i],null);
			if (mOrderLine.getQtyOrdered() == null ||
					(mOrderLine.getQtyOrdered().compareTo(mOrderLine.getQtyInvoiced()) <= 0)) {
				continue;
			}
			String Description = "";
			if (mOrderLine.getM_Product_ID() != 0) {
				MProduct mProduct = new MProduct(m_ctx,mOrderLine.getM_Product_ID(),null);
				Description = mProduct.getValue() + "-" + mProduct.getName();
			} else if (mOrderLine.getC_Charge_ID() != 0) {
				MCharge mCharge = new MCharge(m_ctx,mOrderLine.getC_Charge_ID(),null);
				Description = mCharge.getName();
			}
//			log.warning("prodIDs.size=" + prodIDs.size());
			prodIDs.add(new Integer(mOrderLine.getM_Product_ID()));
			chargeIDs.add(new Integer(mOrderLine.getC_Charge_ID()));
			orderIDs.add(new Integer(mOrderLine.getC_OrderLine_ID()));
			//inOutLineIDs.add(new Integer(mOrderLine.getM_InOutLine_ID()));
			//c_InvoiceIDs.add(new Integer(mOrderLine.getC_InvoiceLine_ID()));
			index = new Integer(index.intValue() + 10);
			BigDecimal RSP = (mOrderLine.getPriceActual().multiply(mOrderLine.getQtyEntered()))
					.setScale(2, BigDecimal.ROUND_HALF_UP);
			Vector<Object> line = new Vector<Object>();
			line.add(index);
			line.add(Description);
			line.add(new Integer(mOrderLine.getQtyEntered().intValue()));
			olddata.add(new Integer(mOrderLine.getQtyEntered().intValue()));
			line.add(mOrderLine.getPriceActual());
			line.add(RSP);
	...
	private BigDecimal calculateTotal(Object[] lmt, int col,
		...
		for (int i = 0; i < rowCnt; i++) {
			if (((Vector) lmt[i]).get(0) != null) {
				...
				} else if (chargeID != null && chargeID > 0) {
...
//	    	olddata.add(new Integer("0"));
	...
	private String createInvoice() {
		...
		if (mOrder == null && data.size() <= 1 ) {
			//FDialog.error(form.getWindowNo(), "Please select Purchase Order");
			cmbPONo.setFocus(true);
			throw new WrongValueException(cmbPONo, "Please select a Purchase Order");
		}
		
		//NCG: #1000394: Duplicate key error on F8 Dazzle Supplier Invoice
		if ( NTierUtils.isInvoiceExists(currentBPartner.getC_BPartner_ID(), txtCustomerRefNo.getText()) ) {
			throw new WrongValueException(txtCustomerRefNo, 
					String.format("The document number '%s' already exists in the invoice table, for this supplier, and this document type.", txtCustomerRefNo.getText()));
		}
		...
		if ( mOrder!= null ) {
			doc.setC_Order_ID(mOrder.getC_Order_ID());
		}
		if (dateInvoiceDate.getValue() != null) {
			doc.setInvoiceDate(new Timestamp(dateInvoiceDate.getValue().getTime()));
		}
		doc.setSOTrx(false);
		MInvoice mInvoice = doc.createInvoiceLines(objs,prodIDs,chargeIDs,orderIDs);
		...
		if (mInvoice != null) {
			...
			//		122);
			doc.setC_DocTypeShipment_ID(122);
			if ( mOrder != null ) {
				doc.createShipment(mOrder,mInvoice,null, mInvoice.getDateInvoiced(),null);
			}
			int invoice_ID = mInvoice.getC_Invoice_ID();
			if (invoice_ID > 0) {
				MInvoice invoice = new MInvoice(m_ctx, invoice_ID, null);
				int m_AD_Process_ID = MProcess.getProcess_ID("POS_Print_InvoiceSuppCr", null);
				...
				org.compiere.process.ProcessInfo pi = new org.compiere.process.ProcessInfo(
						"Supplier Invoice", m_AD_Process_ID);
		...
		//NCG 2014/09/08 Resolve problems with F6 - remove redundant code, replaced with restartWindow in calling function
	...
	public void tableChanged(WTableModelEvent event) {
		...
		Integer chargeID = (Integer) chargeIDs.get(event.getIndex0() - 1);
		if ( chargeID != null && chargeID > 0 ) {
			//NCG Don't allow change of qty on charge, always must be 1
			if (qty!=1) {
				qty = 1;
				line.set(2, new BigDecimal(qty));
			}
			//return;
		}
		if (chargeID == null || chargeID <= 0) {
	...
	/**
	 * @author NCG 2014/10/01
	 * Changed into a function, to resolve NPE
	 * @return
	 */
	private int getSelectedChargeID() {
		//charge_ID_sel = ((WObjRet) cmbCharge.getSelectedItem().getValue()).getIDOfRecord();
		Comboitem obj = (Comboitem) cmbCharge.getSelectedItem();
		if (obj == null ) {
			//Just in case this was hiding something, it can be removed later
			log.warning("Unexpected null value");
			return 0;
		}
		WObjRet obj1 = (WObjRet) obj.getValue();
		if (obj1 == null ) {
			//Just in case this was hiding something, it can be removed later
			log.warning("Unexpected null value 2");
			return 0;
		}
		int id =obj1.getIDOfRecord();
		return id;
	}
