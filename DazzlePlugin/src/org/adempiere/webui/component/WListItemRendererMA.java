 ...
 *
 */
public class WListItemRendererMA implements ListitemRenderer<Object>, EventListener<Event>, ListitemRendererExt
{
    ...
    public ArrayList<ListHeader> m_headers = new ArrayList<ListHeader>();
	 ...
	 *
	 */
	public WListItemRendererMA()
	{
		super();
	}


	public WListItemRendererMA(List< ? extends String> columnNames, List< ? extends Integer> columnWidths)
	{
		super();
		...
		{
			tableColumn = new WTableColumn(columnWidths.get(colInd).intValue());  // set the column width
			colInd++;
	 ...
	 */
	public WListItemRendererMA(List< ? extends String> columnNames)
	{
		super();
		WTableColumn tableColumn;
	...
	public void setM_tableColumns(ArrayList<WTableColumn> m_tableColumns) {
		this.m_tableColumns = m_tableColumns;
	}

	public ArrayList<WTableColumn> getM_tableColumns() {
	...
	private void render(ListItem item, Object data, int index)
	{
		...
		WListboxMA table = null;

		if (item.getListbox() instanceof WListboxMA)
		{
			table = (WListboxMA)item.getListbox();
		}
	 ...
	 */
	private Listcell getCellComponent(WListboxMA table, Object field,
...
//						numberbox.setWidth("100px");
	...
	public void onEvent(Event event) throws Exception
	{
		...
		else if (event.getTarget() instanceof WListboxMA && Events.ON_SELECT.equals(event.getName()))
		{
			WListboxMA table = (WListboxMA) event.getTarget();
			if (table.isCheckmark()) {
