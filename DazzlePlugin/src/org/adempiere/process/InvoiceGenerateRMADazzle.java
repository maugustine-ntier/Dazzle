 ...
 * 
 * Modified NCG 20140730_1000182_DistributionsOnCreditNoteNoDiscount
 * Based on InvoiceGenerateRMA
 * 	Changed to lookup orderline and use prices incl. discount
 */
public class InvoiceGenerateRMADazzle extends SvrProcess
{
    /** Manual Selection        */
    ...
    private MInvoice createInvoice(MRMA rma)
    {
        ...
        //NCG 07 Aug 2014: See VDocument / WDazzleCreditNote. The reference no from the credit note screen is being saved into the Help field. 
        invoice.setPOReference( rma.getHelp() );
        invoice.setRMA(rma);
    ...
    private MInvoiceLine[] createInvoiceLines(MRMA rma, MInvoice invoice)
    {
        ...
        for (MRMALine rmaLine : rmaLines)
        {
            ...
            //NCG Changed here to lookup the order line and use the discounted price entered
            // Otherwise, credit did not have the same discount implication as the invoice.
            if (inoutLineID !=0 ) {
				I_C_OrderLine ordLine =  rmaLine.getM_InOutLine().getC_OrderLine();
				if ( ordLine != null ) {
					//invLine.setPrice(rmaLine.getAmt());
	//				invLine.setPrice(ordLine.getPriceEntered());
					invLine.setPriceEntered( ordLine.getPriceEntered() );
					invLine.setPriceList( ordLine.getPriceList() );
					invLine.setPriceActual( ordLine.getPriceActual() );
			        ...
			        invLine.setLineTotalAmt( invLine.getLineNetAmt().add( invLine.getLineNetAmt() ));
				}
            }
            ...
            if (log.isLoggable(Level.CONFIG))
            	log.config(String.format("InvoiceLine - '%s', qtyinvoiced=%s, priceentered=%s, pricelist=%s, priceactual=%s, linenetamt=%s, linetotalamt=%s", 
            			invLine.toString(), invLine.getQtyInvoiced(), invLine.getPriceEntered(), 
            			invLine.getPriceList(), invLine.getPriceActual(), invLine.getLineNetAmt(), invLine.getLineTotalAmt()));
