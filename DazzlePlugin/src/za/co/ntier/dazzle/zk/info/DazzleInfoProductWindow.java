 ...
 * 
 */
package za.co.ntier.dazzle.zk.info;
...
import org.compiere.model.ZZ_Product;
 ...
 * 
 * @author Neil Gordon
 *  Modified for #20150416_RQ100001_nTier_NCG: Feature for adding and viewing an image relating to a product
 *
 */
public class DazzleInfoProductWindow extends InfoWindow {
	/**
	 * 
	 */
	...
	// #20150416_RQ100001_NCG: Feature for adding and viewing an image relating to a product
	private Panel previewPanel;
	private Iframe preview;
	
//	private WListbox m_tAtp;
	 ...
	 */
	public DazzleInfoProductWindow(int WindowNo, String tableName, String keyColumn,
...
//        productpriceTbl.getModel().addTableModelListener(this);
		...
		// #20150416_RQ100001_NCG: Feature for adding and viewing an image relating to a product
		tab = new Tab(Msg.getMsg(Env.getCtx(), "Image"));
		tabs.appendChild(tab);
		desktopTabPanel = new Tabpanel();
		desktopTabPanel.setHeight("100%");
		...
		desktopTabPanel.appendChild(previewPanel);
		
		//TODO: NCG: Remove comments
//		desktopTabPanel.appendChild(m_tableAtp);
//				chbShowDetailAtp = new Checkbox();
//				chbShowDetailAtp.setLabel(Msg.getMsg(Env.getCtx(), "showDetailAtp", true));
//				chbShowDetailAtp.setTooltiptext(Msg.getMsg(Env.getCtx(), "showDetailAtp", false));
...
//		}
	...
	// #20150416_RQ100001_NCG: Feature for adding and viewing an image relating to a product
	protected void initImageTab() {
		if (log.isLoggable(Level.CONFIG)) log.config("20150416_RQ100001_NCG initImageTab");
		
		Integer M_Product_ID = getSelectedRowKey();
		if (M_Product_ID == null)
			return;
		
		ZZ_Product product = new ZZ_Product(Env.getCtx(), M_Product_ID, null);
		if ( product.getAD_Image_ID() <= 0 ) {
			return;
		}
		
		MImage image = MImage.get(Env.getCtx(), product.getAD_Image_ID());
		
		//		Reset UI
		preview.setSrc(null);
		String contentType = "image/jpeg";
//		String contentType = entry.getContentType();
		AMedia media = new AMedia("Test", null, contentType, image.getData());
